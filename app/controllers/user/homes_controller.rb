class User::HomesController < ApplicationController
  ...
  def top
    ...
    @properties_atmosphere = Property.find(review_with_atmosphere_avg.select { |review| review.atmosphere_avg >= 3.0 }.pluck(:property_id)).first(5)
    review_with_noise_avg = Review.group(:property_id).order("avg(noise) desc").select('property_id, avg(noise) as noise_avg')
    @properties_noise = Property.find(review_with_noise_avg.select { |review| review.noise_avg >= 3.0 }.pluck(:property_id)).first(5)
    ...
    @properties_cleanliness_shared = Property.find(review_with_cleanliness_shared_avg.select { |review| review.cleanliness_shared_avg >= 3.0 }.pluck(:property_id)).first(5)
    ...
    @properties_congestion_shared = Property.find(review_with_congestion_shared_avg.select { |review| review.congestion_shared_avg >= 3.0 }.pluck(:property_id)).first(5)
